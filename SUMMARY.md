# Remediation Summary: anima_fp8/pytorch-preview3-base-fp8-single_device-inference

## Test

`tests/runner/test_models.py::test_all_models_torch[anima_fp8/pytorch-preview3-base-fp8-single_device-inference]`
...
determine the Cosmos model type. The Anima FP8 safetensors file is missing
`net.pos_embedder.dim_spatial_range` (stripped during FP8 quantization), so
diffusers falls back to the `"v1"` (Stable Diffusion 1.x) type, which then tries
to load a config from `stable-diffusion-v1-5/stable-diffusion-v1-5` — a repo that
has no `transformer/config.json`, causing an `OSError`.

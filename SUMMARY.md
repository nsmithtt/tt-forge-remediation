# Remediation Summary: frozen-importlib-swigpyobject-deprecation-warning
...
## Root cause
The string `<frozen importlib._bootstrap>:488: DeprecationWarning: builtin type SwigPyObject has no __module__ attribute` is a Python import-time deprecation warning emitted when a SWIG-based extension module (`SwigPyObject`) is imported without a `__module__` attribute. This warning line was incorrectly captured as a pytest test name by the HF bringup runner's output parser.
...
This is the `SwigPyObject` variant of the same pipeline bug already documented in report/frozen-importlib-swigpypacked-deprecation-warning (the `SwigPyPacked` variant). The bringup runner's output-parsing logic is capturing lines from pytest's warning summary block as test IDs. That block appears at the bottom of pytest output in a section beginning with `warnings summary`, just before the final summary line. The runner should skip any captured "test name" that does not begin with `tests/`.
